...
export default function HeadScrollExperience() {
  ...
  const scheduleSeek = useCallback((progress: number) => {
    ...
    rafRef.current = requestAnimationFrame(() => {
      ...
      const frameTime = Math.round(targetTimeRef.current / frameStep) * frameStep;
      const nextTime = Math.min(frameTime, maxTime);

      if (Math.abs(video.currentTime - nextTime) >= frameStep * 0.75) {
  ...
  const primeVideo = useCallback(async (fromGesture = false) => {
    ...
    try {
      ...
      const initialTime = Math.min(
        ...
        Math.max(0, (video.duration || durationRef.current) - 1 / 24),
      );
          ...
          <video
            ...
            onSeeked={() => {
              if (seekPendingRef.current) {
                seekPendingRef.current = false;
                scheduleSeek(
                  durationRef.current > 0 ? targetTimeRef.current / durationRef.current : 0,
                );
              }
            }}
            onError={(event) => {
