...
function CopyBlock({
  ...
  const positions = {
    left:
      "left-4 right-4 items-start text-left sm:left-8 sm:right-auto sm:w-[min(94vw,1500px)] lg:left-[5vw]",
    center:
      "left-4 right-4 items-center text-center sm:left-1/2 sm:right-auto sm:w-[min(96vw,1600px)] sm:-translate-x-1/2",
    right:
      "left-4 right-4 items-start text-left sm:left-auto sm:right-8 sm:w-[min(94vw,1500px)] sm:items-end sm:text-right lg:right-[5vw]",
  };

  return (
    <motion.div
      style={{ opacity, y, filter }}
      className={`copy-block copy-block-${position} pointer-events-none absolute top-[54%] z-30 flex -translate-y-1/2 flex-col sm:top-1/2 ${positions[position]}`}
    >
